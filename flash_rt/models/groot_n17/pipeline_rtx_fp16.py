"""GR00T N1.7 full-FP16 forward pipeline — pointer-only forward functions.

Full-FP16 baseline variant of ``pipeline_thor.py``: every FP8 GEMM
(``quantize_fp8_static_fp16`` + ``fp8_nn_*``) is replaced by a single
``fp16_nn`` on FP16 weights, and the DiT (bf16 in the FP8 path) is run in
FP16 too, so the whole N1.7 path is uniform FP16. No activation
calibration. Exists as an A/B precision reference against the FP8 path;
the FP8 path (``pipeline_thor.py``) is left untouched.
...
def qwen3vl_vit_forward(gemm, fvk, bufs, weights, dims,
    ...
    """24-layer Qwen3-VL ViT (full-FP16 GEMMs, multi-view batched FMHA).
    ...
    for li in layer_iter:
        ...
        # ── 3 split FP16 GEMMs + bias ───────────────────────────────────
        gemm.fp16_nn(xn_ptr, int(weights["q_w"][li]), Q_ptr, S, D, D, int(stream))
        fvk.add_bias_fp16(Q_ptr, int(weights["q_b"][li]), S, D, int(stream))
        gemm.fp16_nn(xn_ptr, int(weights["k_w"][li]), K_ptr, S, D, D, int(stream))
        fvk.add_bias_fp16(K_ptr, int(weights["k_b"][li]), S, D, int(stream))
        gemm.fp16_nn(xn_ptr, int(weights["v_w"][li]), V_ptr, S, D, D, int(stream))
        ...
        # ── O projection (FP16) ─────────────────────────────────────────
        gemm.fp16_nn(O_ptr, int(weights["o_w"][li]), o_proj_out, S, D, D, int(stream))
        ...
        # ── FF: D → FF (GELU) → D (FP16) ───────────────────────────────
        gemm.fp16_nn(xn_ptr, int(weights["fc1_w"][li]), fc1_out_ptr, S, FF, D, int(stream))
        fvk.add_bias_fp16(fc1_out_ptr, int(weights["fc1_b"][li]), S, FF, int(stream))
        fvk.gelu_inplace_fp16(fc1_out_ptr, S * FF, int(stream))
        gemm.fp16_nn(fc1_out_ptr, int(weights["fc2_w"][li]), o_proj_out, S, D, FF, int(stream))
...
def deepstack_merge_forward(gemm, fvk, bufs, weights, dims,
    ...
    for j in range(3):
        ...
        # 2. fc1 (FP16) + bias + GELU: (Nout, Dmid) × (Dmid, Dmid) → (Nout, Dmid)
        gemm.fp16_nn(ln_out, int(weights["fc1_w"][j]), fc1_out,
        ...
        # 3. fc2 (FP16) + bias: (Nout, Dmid) × (Dmid, Dout) → (Nout, Dout)
        gemm.fp16_nn(fc1_out, int(weights["fc2_w"][j]), out_ptr,
...
def qwen3vl_llm_forward(gemm, fvk, bufs, weights, dims,
    ...
    for li in layer_iter:
        ...
        # ── 3 split FP16 GEMMs (no bias — Qwen3 QKV) ───────────────────
        gemm.fp16_nn(xn_ptr, int(weights["q_w"][li]), Q_ptr,
                     S, NHQ * HD, D, int(stream))
        gemm.fp16_nn(xn_ptr, int(weights["k_w"][li]), K_ptr,
                     S, NHKV * HD, D, int(stream))
        gemm.fp16_nn(xn_ptr, int(weights["v_w"][li]), V_ptr,
                     S, NHKV * HD, D, int(stream))
        ...
        # ── O projection (FP16) ─────────────────────────────────────────
        gemm.fp16_nn(int(slots["O"]), int(weights["o_w"][li]), o_out_ptr,
        ...
        # ── gate / up FP16 GEMMs ──────────────────────────────────────
        gemm.fp16_nn(xn_ptr, int(weights["gate_w"][li]), gate_ptr,
                     S, FF, D, int(stream))
        gemm.fp16_nn(xn_ptr, int(weights["up_w"][li]), up_ptr,
                     S, FF, D, int(stream))
        # ── SiLU(gate) * up → gate (compose: silu in-place then mul) ──
        fvk.silu_inplace_fp16(gate_ptr, S * FF, int(stream))
        fvk.mul_fp16(gate_ptr, up_ptr, gate_ptr, S * FF, int(stream))
        # ── down GEMM (FP16) ──────────────────────────────────────────
        gemm.fp16_nn(gate_ptr, int(weights["down_w"][li]), o_out_ptr,
                     S, D, FF, int(stream))
...
def vl_self_attn_forward(gemm, fvk, bufs, weights, dims,
    ...
    for li in layer_iter:
        ...
        # ── Q / K / V GEMMs (FP16) + bias ───────────────────────────────
        gemm.fp16_nn(xn_ptr, int(weights["q_w"][li]), Q_ptr, T, D, D, int(stream))
        fvk.add_bias_fp16(Q_ptr, int(weights["q_b"][li]), T, D, int(stream))
        gemm.fp16_nn(xn_ptr, int(weights["k_w"][li]), K_ptr, T, D, D, int(stream))
        fvk.add_bias_fp16(K_ptr, int(weights["k_b"][li]), T, D, int(stream))
        gemm.fp16_nn(xn_ptr, int(weights["v_w"][li]), V_ptr, T, D, D, int(stream))
        ...
        # ── O projection (FP16) ─────────────────────────────────────────
        gemm.fp16_nn(O_ptr, int(weights["o_w"][li]), o_proj_out, T, D, D, int(stream))
        ...
        # ── FF: 2048 → 8192 (GELU) → 2048 (FP16) ───────────────────────
        gemm.fp16_nn(xn_ptr, int(weights["fc1_w"][li]), fc1_out_ptr,
                     T, FF, D, int(stream))
        fvk.add_bias_fp16(fc1_out_ptr, int(weights["fc1_b"][li]), T, FF, int(stream))
        fvk.gelu_inplace_fp16(fc1_out_ptr, T * FF, int(stream))
        gemm.fp16_nn(fc1_out_ptr, int(weights["fc2_w"][li]), o_proj_out,
                     T, D, FF, int(stream))
...
def dit_forward(gemm, fvk, bufs, weights, dims,
    ...
    attend_text_every_n_blocks=2). Full-FP16 GEMMs (this baseline runs the
    DiT in FP16; the FP8 production path runs it in bf16).
    ...
    for li in layer_iter:
        ...
        gemm.fp16_nn(xn_ptr, int(weights["q_w"][li]),
                      Q_ptr, Sa, D, D, int(stream))
        fvk.add_bias_fp16(Q_ptr, int(weights["q_b"][li]),
                           Sa, D, int(stream))

        if is_self:
            gemm.fp16_nn(xn_ptr, int(weights["k_w"][li]),
                          K_ptr, Sa, D, D, int(stream))
            fvk.add_bias_fp16(K_ptr, int(weights["k_b"][li]),
                               Sa, D, int(stream))
            gemm.fp16_nn(xn_ptr, int(weights["v_w"][li]),
                          V_ptr, Sa, D, D, int(stream))
            fvk.add_bias_fp16(V_ptr, int(weights["v_b"][li]),
                               Sa, D, int(stream))
        ...
        gemm.fp16_nn(O_ptr, int(weights["o_w"][li]),
                      o_out_ptr, Sa, D, D, int(stream))
        fvk.add_bias_fp16(o_out_ptr, int(weights["o_b"][li]),
                           Sa, D, int(stream))
        fvk.residual_add_fp16(h_ptr, o_out_ptr, Sa * D, int(stream))
        ...
        gemm.fp16_nn(xn_ptr, int(weights["ff_proj_w"][li]),
                      ff_out_ptr, Sa, FF, D, int(stream))
        fvk.add_bias_fp16(ff_out_ptr, int(weights["ff_proj_b"][li]),
                           Sa, FF, int(stream))
        fvk.gelu_inplace_fp16(ff_out_ptr, Sa * FF, int(stream))
        gemm.fp16_nn(ff_out_ptr, int(weights["ff_down_w"][li]),
                      o_out_ptr, Sa, D, FF, int(stream))
        fvk.add_bias_fp16(o_out_ptr, int(weights["ff_down_b"][li]),
                           Sa, D, int(stream))
        fvk.residual_add_fp16(h_ptr, o_out_ptr, Sa * D, int(stream))
...
def embodiment_state_encode(gemm, fvk, bufs, weights, dims, *,
    ...
    gemm.fp16_nn(
        ...
        int(bufs["h1"]), M, h_dim, in_dim, int(stream),
    )
    fvk.add_bias_fp16(int(bufs["h1"]), int(weights["l1_b"]), M, h_dim, int(stream))
    fvk.relu_inplace_fp16(int(bufs["h1"]), M * h_dim, int(stream))
    gemm.fp16_nn(
        int(bufs["h1"]), int(weights["l2_w"]),
        int(bufs["out"]), M, out_dim, h_dim, int(stream),
    )
    fvk.add_bias_fp16(int(bufs["out"]), int(weights["l2_b"]), M, out_dim, int(stream))
...
def embodiment_action_encode(gemm, fvk, bufs, weights, dims, *,
    ...
    gemm.fp16_nn(
        ...
        int(bufs["i1"]), T, H, A, int(stream),
    )
    fvk.add_bias_fp16(int(bufs["i1"]), int(weights["b1"]), T, H, int(stream))
    ...
    gemm.fp16_nn(
        ...
        int(bufs["i2"]), T, H, Cat, int(stream),
    )
    fvk.add_bias_fp16(int(bufs["i2"]), int(weights["b2"]), T, H, int(stream))
    fvk.relu_inplace_fp16(int(bufs["i2"]), T * H, int(stream))

    # Layer 3: (T, H) → (T, H)
    gemm.fp16_nn(
        int(bufs["i2"]), int(weights["W3"]),
        int(bufs["out"]), T, H, H, int(stream),
    )
    fvk.add_bias_fp16(int(bufs["out"]), int(weights["b3"]), T, H, int(stream))
...
def embodiment_action_decode(gemm, fvk, bufs, weights, dims, *,
    ...
    gemm.fp16_nn(
        ...
        int(bufs["h"]), T, h_dim, in_dim, int(stream),
    )
    fvk.add_bias_fp16(int(bufs["h"]), int(weights["l1_b"]), T, h_dim, int(stream))
    fvk.relu_inplace_fp16(int(bufs["h"]), T * h_dim, int(stream))
    gemm.fp16_nn(
        int(bufs["h"]), int(weights["l2_w"]),
        int(bufs["velocity"]), T, out_dim, h_dim, int(stream),
    )
    fvk.add_bias_fp16(int(bufs["velocity"]), int(weights["l2_b"]), T, out_dim, int(stream))
